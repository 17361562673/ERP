...
<script type="text/javascript">
    $(function() {
        ...
        //校验代码(对必填项的校验)
        //通过属性选择器找到每个要校验的
        $("#empEditForm").find("[regr]").keyup(function () {
            //获得它的值
            var val=$(this).val();
        ...
        //校验代码(对非必填项的校验)
        //通过属性选择器找到每个要校验的
        $("#empEditForm").find("[reg]").keyup(function () {
            var valStr=$(this).val();
    ...
    function validForm() {
        ...
        //校验代码(对必填项的校验)
        //通过属性选择器找到每个要校验的
        $("#empEditForm").find("[regr]").each(function () {
            //获得它的值
            var val=$(this).val();
            //获得每一个必填项的正则表达式(通过属性获得)
            var regStr=$(this).attr("regr");
            ...
            if (!regr.test(val)) {
                ...
                if (vName == "emp.username") {
                    ...
                    //获得原始用户名(和新的username做对比，如果不一样就校验如果一样就不校验)
        ...
        //校验代码(对非必填项的校验)
        //通过属性选择器找到每个要校验的
        $("#empEditForm").find("[reg]").each(function () {
            var valStr=$(this).val();
            var regStr=$(this).attr("reg");
            var tipStr=$(this).attr("tip");
            var reg=new RegExp(regStr);
...
<div class="content-right">
    <div class="content-r-pic_w">
        <div style="margin:8px auto auto 12px;margin-top:6px">
            <span class="page_title">员工维护---员工修改</span>
        </div>
    </div>
    <div class="content-text">
        <div class="square-order">
            <form  id="empEditForm" action="${path}/ajax_emp_edit.action" method="post">
                <div style="border:1px solid #cecece;">
                    <%--为了校验用户名不和自己重复 使用<s:property value="emp.username"/>也是可以回显的--%>
                    <input id="username" type="hidden" value="<s:property value="emp.username"/>">
                        <%--传到后台的empId--%>
                    <input name="emp.empId" type="hidden" value="<s:property value="emp.empId"/>">
